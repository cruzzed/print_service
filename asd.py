...
class QRPDFPrinter:
    ...
    def create_widgets(self):
        ...
        label_combo = ttk.Combobox(config_frame, textvariable=self.label_printer_var, width=30)
        ...
        receipt_combo = ttk.Combobox(config_frame, textvariable=self.receipt_printer_var, width=30)
        ...
        config_btn_frame.grid(row=0, column=2, rowspan=2, padx=(5, 0))
    ...
    def send_to_printer(self, file_path, print_type):
        ...
        try:
            ...
            if system == "Windows":
                ...
                # Add default options based on print type
                if print_type == "label":
                    if not any("media=" in opt for opt in printer_options):
                        cmd.extend(["-o", "media=Custom.2x1in"])
                elif print_type == "receipt":
                    if not any("media=" in opt for opt in printer_options):
                        cmd.extend(["-o", "media=Receipt"])
                ...
                if print_type == "label":
                    if not any("media=" in opt for opt in printer_options):
                        cmd.extend(["-o", "media=Custom.51x25mm"])
                elif print_type == "receipt":
                    if not any("media=" in opt for opt in printer_options):
                        cmd.extend(["-o", "media=Receipt"])
